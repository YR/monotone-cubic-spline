...
$m['src/index'].exports = {
   ...
   */
  points(points) {
    const tgts = srcindex__tangents(points);
 ...
 * @returns {Array}
 */
function srcindex__tangents(points) {
  const m = srcindex__finiteDifferences(points);
  ...
  for (let i = 0; i < n; i++) {
    d = srcindex__slope(points[i], points[i + 1]);

    if (Math.abs(d) < srcindex__ε) {
      m[i] = m[i + 1] = 0;
    } else {
      a = m[i] / d;
      b = m[i + 1] / d;
 ...
 * @returns {Number}
 */
function srcindex__slope(p0, p1) {
 ...
 * @returns {Array}
 */
function srcindex__finiteDifferences(points) {
  const m = [];
  let p0 = points[0];
  let p1 = points[1];
  let d = m[0] = srcindex__slope(p0, p1);
  let i = 1;

  for (let n = points.length - 1; i < n; i++) {
    p0 = p1;
    p1 = points[i + 1];
    m[i] = (d + (d = srcindex__slope(p0, p1))) * 0.5;
  }
  m[i] = d;
